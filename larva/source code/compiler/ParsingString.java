...
public class ParsingString{
	...
	public ParsingString parseWrapper(String title,boolean strict) throws ParseException
	{
		...
		//string till first open curly bracket
		String stringTillBracket = string.substring(0, start).trim();
		
		if (stringTillBracket.indexOf(title)!=0
				&& stringTillBracket.indexOf(title.toUpperCase())!=0)
		{
			if  (strict)
		...
		else //either or gave a 0 index
		{
			params = stringTillBracket.substring(title.length()).trim();
			start++;			
		}
