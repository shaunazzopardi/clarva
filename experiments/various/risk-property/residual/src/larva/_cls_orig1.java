...
int _state_id_riskProperty = 140;
...
if (0==1){}
else if (_state_id_riskProperty==140){
		if (1==0){}
		else if ((_occurredEvent(_event,238/*activate*/))){
		
		_state_id_riskProperty = 139;//moving to state activated
		_goto_riskProperty(_info);
		}
		else if ((_occurredEvent(_event,240/*payFromCard_exit*/))){
		
		_state_id_riskProperty = 136;//moving to state bad
		_goto_riskProperty(_info);
		}
}
}
...
switch(_state_id){
case 136: if (_mode == 0) return "bad"; else return "!!!SYSTEM REACHED BAD STATE!!! bad "+new _BadStateExceptionorig().toString()+" ";
case 138: if (_mode == 0) return "blacklistedHighRisk"; else return "blacklistedHighRisk";
case 140: if (_mode == 0) return "start"; else return "start";
case 137: if (_mode == 0) return "blacklistedLowRisk"; else return "blacklistedLowRisk";
case 139: if (_mode == 0) return "activated"; else return "activated";
