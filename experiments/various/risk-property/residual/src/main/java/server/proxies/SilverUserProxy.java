...
public class SilverUserProxy {

//	public static SilverUserProxy proxy;
	public static SilverUserInfo external = new SilverUserInfo(-1);
	public static UserInterface intrf = UserInterface.ui;
		
//	public SilverUserProxy(){
//		intrf;
//		proxy = this;
//	}

	public static synchronized void loggedInMenu(UserUI userUI, SilverUserInfo u, int sid){//, long end){
		boolean exit = false;
				...
				case 4: //de-register
					...
					if(userIDs.size() > 0){
						...
						if(args.size() > 0){
							...
							System.out.println("User " + uid + " deregistered.");
						}
					}
					break;
			case 5: //authenticate
				...
				if(userIDs.size() > 0){
					...
					if(args.size() > 0){
						...
						System.out.println("User " + uid + " requested authentication.");
					}
				}
				break;
//				default: intrf.logout(u, sid); exit = true; acts = 0; break;
			}
	...
	public static synchronized void accountMenu(UserUI userUI, SilverUserInfo u, int sid){//, long end){
		boolean exit = false;
...
//								System.out.println("User " + uid + " closed account " + sid + ".");
//							}
//
//						}
//						break;
	...
	public static synchronized void cardMenu(UserUI userUI, SilverUserInfo u, int sid){//, long end){
		boolean exit = false;
...
//							
//							if(args.size() > 1){
//								if(intrf.payToCompanyFromCard(u, (int) args.get(0), sid, (int) args.get(1), transactionAmount)){
...
//						break;
	...
	public static synchronized void transactionWhitelistedMenu(UserUI userUI, SilverUserInfo u, int sid){//, long end){
		boolean exit = false;
...
//			options.add("(2) to make payment from account");
			options.add("(2) to transfer money between your own accounts");
			options.add("(3) to deposit money into your account");
//			options.add("(4) to exit");
//			options.add("(5) to transfer money to another user's account");
...
//						
//						List<Object> userIDs = new ArrayList<Object>();
//						intrf.ts.users.forEach(user -> userIDs.add(user.getId()));
//						acceptableValues.add(userIDs);
//
//						argTypes.clear();
//						argTypes.add(Integer.class);
...
//						break;
	...
	public static synchronized void transactionGreylistedMenu(UserUI userUI, SilverUserInfo u, int sid){//, long end){
		boolean exit = false;
...
//			options.add("(2) to make payment from account");
			options.add("(2) to transfer money between your own accounts");
//			options.add("(3) to exit");
//			options.add("(4) to deposit money into your account");
//			options.add("(5) to transfer money to another user's account");
...
//						break;
	...
	public static synchronized void transactionBlacklistedMenu(UserUI userUI, SilverUserInfo u, int sid){//, long end){
		boolean exit = false;
...
//			options.add("(1) to make a card payment");
//			options.add("(2) to make payment from account");
			options.add("(1) to transfer money between your own accounts");
//			options.add("(2) to exit");
//			options.add("(4) to deposit money into your account");
//			options.add("(5) to transfer money to another user's account");
